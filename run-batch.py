...
if __name__ == '__main__':
	for entry in os.scandir(arguments_strIn):
		print(f'processing {entry.name} ... \n')
		tenInput = torch.FloatTensor(numpy.ascontiguousarray(numpy.array(PIL.Image.open((entry.path)))[:, :, ::-1].transpose(2, 0, 1).astype(numpy.float32) * (1.0 / 255.0)))
		tenOutput = estimate(tenInput)
		out_path = os.path.join(arguments_strOut, entry.name)
		PIL.Image.fromarray((tenOutput.clamp(0.0, 1.0).numpy().transpose(1, 2, 0)[:, :, 0] * 255.0).astype(numpy.uint8)).save(out_path)
# end